...
export default function Forget() {
  ...
  const Forget = () => {
    ...
    if (Email == "") {
      ...
      formdata.append("sendstatus", 'teacher');
      ...
      fetch(`${baseUrl.baseUrl}webapi/forgettokenCheck`, requestOptions)
        ...
        .then(result => {
          ...
          if (result.status) {
            // <NewPassword id={Id}/>
            Router.push({
              pathname: '/teacher/auth/forgetpassword/newPassword',
              query: { id: Id }
            })
            ...
            <div className="sign_form">
              ...
              <form>
                <div className="ui search focus mt-50">
                  ...
                  {CodeFeild ? <>
                    <label className="label25 text-left">Verification Code<span className="text-danger">*</span></label>
                    <div className="ui left icon input swdh95">
                      <input className="prompt srch_explore" onChange={(e) => handleInputChange(e, setCode)} type="number" name="Verification code" defaultValue={Code} required maxLength={64} placeholder="Enter Verification Code"  />
                      {/* <i className="uil uil-envelope icon icon2" /> */}
                    </div>
                  </>
                    : null}
                ...
                {CodeFeild ?
                  <button onClick={Confirmation} className="login-btn" type="button" disabled={Code == "" ? true : false}>Verify</button>
                  :
                  <button onClick={Forget} className="login-btn" type="button">Reset Password</button>
                }
              </form>
              <Link href={"/teacher/auth/login"} passHref><p className="mb-0 mt-30 sgntrm145">Go Back <a href="#">Sign In</a></p></Link>
            </div>
